...
bot = TeleBot(cfg["API_KEY"], parse_mode="MARKDOWN")
...
def send_losung(message: Message, date_query: datetime.date = None) -> None:
...
def send_losung_date(message: Message) -> None:
    ...
    if len(request) < 2:
        ...
        bot.send_message(message.chat.id, f"Benutzung: /datum {yesterday}")
        return
